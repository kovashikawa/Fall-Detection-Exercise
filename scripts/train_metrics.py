...
logger = setup_logger('train_with_metrics', f'training_{timestamp}')
...
def evaluate_classifier_metrics(model, loader, device):
    model.eval()
    y_true, y_pred = [], []
    with torch.no_grad():
        for x_batch, y_batch in loader:
...
def evaluate_regressor_metrics(model, loader, device):
    model.eval()
    y_true, y_pred = [], []
    with torch.no_grad():
        for x_batch, y_batch in loader:
...
def main():
    ...
    f1, auc = evaluate_classifier_metrics(classifier, val_loader, device)
    logger.info(f'Classifier - F1 Score: {f1:.4f}, ROC-AUC: {auc:.4f}')

    mae, r2 = evaluate_regressor_metrics(regressor, val_loader, device)
    logger.info(f'Regressor - MAE: {mae:.4f}, R²: {r2:.4f}')
